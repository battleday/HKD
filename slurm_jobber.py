...
def get_sbatch_cmd(n_tasks=1,
    ...
    # time limit
    cmd = update_cmd(cmd, '--time {}'.format(minutes))
...
def get_out_fn(py_params, mode=MODE, 
    ...
    for k, key in enumerate(py_params.keys()):
        ...
        if mode == 'wrap':
            ...
            print('just mpid {} for outfile'.format(mpid))
...
#            conf_fn = get_out_fn(arg_grid, just_fn=True, mpid=file_idx) + '.conf'
            conf_fn = '{}'.format(file_idx) + '.conf'

            curr_sbatch_cmd = get_sbatch_cmd(n_tasks=MP_BATCH_SIZE,
                                             **sbatch_args)
            curr_sbatch_cmd = update_cmd(curr_sbatch_cmd, out_fn)
            if MODULE != "none":
                curr_sbatch_cmd = curr_sbatch_cmd + ['module load {}'.format(MODULE)] + ['conda activate {}'.format(ENVIRONMENT)]
            else:
                curr_sbatch_cmd = curr_sbatch_cmd + ['conda activate {}'.format(ENVIRONMENT)]
